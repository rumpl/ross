...
pub async fn ensure_essential_dirs(rootfs: &Path) -> Result<(), ShimError> {
    ...
    // Set up DNS configuration for libkrun's TSI networking
    // TSI intercepts socket syscalls, so we need valid DNS servers.
    // Use public DNS servers that will work through TSI.
    let resolv_conf = rootfs.join("etc/resolv.conf");
    let dns_config = "# Generated by ross for libkrun TSI networking\n\
                      nameserver 8.8.8.8\n\
                      nameserver 8.8.4.4\n\
                      nameserver 1.1.1.1\n";
    fs::write(&resolv_conf, dns_config).await?;
...
pub async fn create_minimal_rootfs(target_dir: &Path) -> Result<(), ShimError> {
    ...
    fs::write(
        target_dir.join("etc/resolv.conf"),
        "# Generated by ross for libkrun TSI networking\nnameserver 8.8.8.8\nnameserver 8.8.4.4\nnameserver 1.1.1.1\n",
    )
    .await?;
